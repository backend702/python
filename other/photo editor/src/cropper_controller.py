...
class MyApp(App):
    ...
    def crop_and_load_next(self):
        ...
        if not self.cropper:
            ...
            self.layout.aspect_ratio_field.bind(on_text_validate=partial(new_aspect_ratio, self.cropper))
            ...
            for size_input in (self.layout.width_field, self.layout.height_field):
                size_input.bind(on_text_validate=partial(
...
def set_dimension(dimension, cropper, cropper_layout, instance):
...
def new_aspect_ratio(cropper, textinput):
...
if __name__ == '__main__':
    imgs_data = get_imgs('images')
    if len(imgs_data) < 1:
        print('No images were found inside the folder "images"')
        input()
        sys.exit()
    MyApp(imgs_data).run()
...
#     self.image_widget.source='images/1.png'